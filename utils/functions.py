...
def data_process(input_doc, relational_alphabet, tokenizer):
    ...
    for i in range(len(lines)):
        token_sent = [tokenizer.cls_token] + tokenizer.tokenize(remove_accents(lines[i]["sentText"])) + [
            tokenizer.sep_token]
        ...
        for triple in triples:
            ...
            assert tail_end_index >= tail_end_index
...
def generate_triple(output, info, args, num_classes):
    _Pred_Triple = collections.namedtuple(
        ...
        ["pred_rel", "rel_prob", "head_start_index", "head_end_index", "head_start_prob", "head_end_prob",
         "tail_start_index", "tail_end_index", "tail_start_prob", "tail_end_prob"]
    )
    ...
    if pred_rel.pred_rel != num_classes:
        if pred_head and pred_tail:
            ...
            return _Pred_Triple(pred_rel=pred_rel.pred_rel, rel_prob=pred_rel.rel_prob,
                                ...
                                tail_start_index=tail.start_index, tail_end_index=tail.end_index,
                                tail_start_prob=tail.start_prob, tail_end_prob=tail.end_prob)
        else:
            return
    else:
        return
    ...
    for i in range(len(sent_idxes)):
        ...
        for j in range(len(target[i]["relation"])):
            gold[sent_idxes[i]].append(
                (target[i]["relation"][j].item(), target[i]["head_start_index"][j].item(),
                 target[i]["head_end_index"][j].item(), target[i]["tail_start_index"][j].item(),
