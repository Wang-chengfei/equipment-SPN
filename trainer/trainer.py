    ...
    def train_model(self):
        ...
        for epoch in range(self.args.max_epoch):
            ...
            if is_train:
                ...
                # for batch_id in range(int(total_batch / 10)):
            ...
            # 记录测试准确率
            self.F1s.append(f1)
            with open(self.args.generated_result_directory + self.args.train_result_name, 'w') as f:
                json.dump(self.F1s, f)
            self.writer.add_scalar('Test/F1', result['f1'], epoch)
            self.writer.add_scalar('Test/Precision', result['precision'], epoch)
            self.writer.add_scalar('Test/Recall', result['recall'], epoch)
            if f1 > best_f1:
                print("Achieving Best Result on Validation Set.", flush=True)
                torch.save(self.model.state_dict(), self.args.generated_param_directory + "%s_%s_epoch_%d_f1_%.4f.pt" % (self.args.model_name, self.args.dataset_name, epoch, result['f1']))
                best_f1 = f1
